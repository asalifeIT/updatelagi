...
export class ServiceService {
  ...
  loginApi(credentials, type) {
    ...
    return this.http
      .post(this.API_URL + type, credentials, {
        headers: headers,
        observe: "response",
      })
      .pipe(
        tap((Data) => {
          ...
          if (Data.status == 200) {
            localStorage.setItem(
              this.TOKEN_KEY,
              JSON.stringify(this.DataLogin.access_token)
            );
            localStorage.setItem(
              this.REFRESH_TOKEN_KEY,
              JSON.stringify(this.DataLogin.refresh_token)
            );
            localStorage.setItem(
              this.ROLE,
              JSON.stringify(this.DataLogin.roles[1])
            );
            localStorage.setItem("user", JSON.stringify(this.DataLogin));
            localStorage.setItem("roles", JSON.stringify(this.DataLogin.roles));
            localStorage.setItem(
              "userName",
              JSON.stringify(this.DataLogin.user.name)
            );
  ...
  RegisterApi(credentials, type) {
    ...
    return this.http
      .post(this.API_URL + type, credentials, { headers: headers })
      .pipe(
        tap((Data) => {
          this.DataLogin = Data;
  ...
  submitaduan(form, url) {
    ...
    return this.http
      .post(this.API_URL + url, form, { headers: headers, observe: "response" })
      .pipe(
        tap((Data) => {
          this.DataLogin = Data;
  ...
  getRecord(url) {
    ...
    return this.http
      .get(this.API_URL + url, { headers: headers, observe: "response" })
      .pipe(
        tap((Data) => {
  ...
  updateaduan(form, url) {
    ...
    return this.http
      .put(this.API_URL + url, form, { headers: headers, observe: "response" })
      .pipe(
        tap((Data) => {
  ...
  isHasAccess(_feature: string, _section: string, _action: string) {
    ...
    return !!action.hasAccess.find(
      (item) => JSON.stringify(item) === JSON.stringify(roleUser)
    );
  }
